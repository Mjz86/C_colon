...
an addressless memory region with lifetime L cannot be pointed to by any pointer or reference during L, 
an unaddressable memory region with lifetime L must not be addressed in L, meaning that if a pointer is used during L it is not within M, 
an unaddressed memory region must not be addressed while the qualifier is present, 
an addressed memory region may have pointers poining to it be used.
