...
this ABI aims to have minimal compatibility with the Itanium ABI, such that an `extern "c++" represent_cxx` statement can make most code usable in a c++ platform,
 ...
 some concepts may be excluded from such inclusions, for instance, a c++ type's declaration must be expressible in c colon and vice versa .
...
but code that is not truthfully const correct cannot assume stability, and therefore can exist in the unstable region, allowing the ability to express what a c++ constant is(all necessary for `represent_cxx` to say the correct story ).
...
for a pointer p declared within its lifetime L,  to an stable const region of memory r, if a stable value const v loaded from address a originating from p is loaded from memory, then until the end of L, the expression `std::memcmp(std::addressof(v),a,sizeof(v))==0` must be true( the values at that memory region must not be modified), otherwise the behavior is undefined.
 ...
 `stabilized` (default)/ `unstabilized`:
...
unrestricted disables the exclusive mutability borrow rule in the compiler.
 ...
 if the pointer P has the region as mut, for the duration LP, only P must represent the address range of M, meaning that for any other pointer Q with address A within M, if Q is used in lifetime LP, the behaviour is undefined.
 


addressed/unaddressed/unaddressable(default in E colon mode)/addressless:
an addressless object O with lifetime L cannot be  aliased( sub objects are may or may not be aliased based on qualification) by any pointer or reference during L, 
an unaddressable object O with lifetime L must not be addressed in L, meaning that if a pointer is used during L it is not aliasing O , 
an unaddressed object O must not be addressed while the qualifier is present, 
an addressed object O may have pointers poining to it be used.
if the above is not the case the behavior is undefined.
...
* note :
dynamic structures  are not counted as pointers even if their abi has internal pointers generated by the compiler,
...
this means that all linkable binaries with this qualifier must be availabel when the program starts,
...
while most symbols i mentioned are functions, static variables and other stuff can have symbols as well, and these may not be trivially destructible or trivially bit copyable.
...
- `dllinited` on objects : 
 the object static (de)initialization sequence happens unconditionally,
...
- `moinline`:
 ...
 so, we need a way to combine this without the hassle of definition duplications ( least the duplication being optional instead of mandatory),
...
value's storage is valid in the current expression, but the address may change at any time with relocation, but for a given unique owning reference to the register its the same
...
unaligned data is almost always trivially relocatable, trivially destructible and without pointers, basically, the requirements for a safe bit cast)to be useful, because its probably a network or external packet anyway.
...
if no rtti is emitted, dynamic cast will return nullptr on all cases ( but not on cast to byte/void pointer because the offset is availabel, and in cases where a base object is accessed it would need to be rewriten as a static cast anyway).
...
a dyn object member is a pointer to that member, if nullabel it can be null, and access to it would result in an implicit contract violation.
the cow qualification can be used on this object, creating any mutable reference will make the cow copy a unique reference to that copy, creating a copy of the pointer does a shallow cow copy using an implicit atomic reference counter before or after the object( can be padded in the compiler if alignment requirements say so).
 creating cow copies  will convert the qualifier of mut to const, 
 to undo this one must  do  ownerization  by calling `std::ownerize_cow`  removing or adding cow or nullable has similar  library  functions.
...
- `fastdyncaller(default if nothing is known about callee)/fastdyncallee/fastdyncall(default if the synth and summary  files  are present for a dll call )`:
...
it is architecture  defined whether any registers  are explicitly forcefully put in the used set( it would help dynamic callee functions  if some registers are reserved )  or forcefully excluded from the used set ( it would help dynamic  caller functions  if some registers are reserved ).

a  `fastdyncall(info)` convention is talor made for a function using profile guides and summary information synthesized into a custom  usage plan,
 the usage affects the function pointer abi and its hash, it is useful for maximizing dynamic library call performance .
if a dll definition file has this information,  the dll calls will have this convention.

static function calls are susceptible to register reassignment and so the calling convention is not explicitly defined but is coherent for that function's static identity. 
 
 * note:
  including a module from a dll needs a dll definition file to add those entries to dllimport symbols in the table,  similar  to a header file in C++  but automatically generated by the dll distribution system , its similar to a precompiled module.
...
---
 ...
 its similar to how cxx does coroutine frame elision, but this time the stack frame is also availabel. 
...
struct castation_table_ref_t {
 ...
 // initialization phase, note that dllimport vtable layouts are partially
 // known, but their content this is used to access the cast table
 const base_castation_table_t *castation_ptr;
 // relative to most derived, used in cast to void/byte/bit pointer or access
...
// dllhidden is default, however, we can use :
...
// the nodes are ordered by sorting the abi hashes so the binary search  and other tricks hold 
// 
...
for ensuring a sane order of initialization between the modules of different names and versions, 
...
may be read from after initialization or written to , but its undefined if the caller reads these after the call, except when initialized again.
...
an `uninitialized xvaluexpr  ` argument can be used as a dummy in val argument that isnt initialized in nither caller nor callee,
...
or an `uninitialized ivaluexpr  ` if the caller needs an extra callee saved register ( the in category)
...
on de-initialization, the dll de initilizer is called.
 note that this section of the specification may need to change a bit, but generally the goal is to make module initialization predictable based on the source code and each module is initialized as if wrapped by a synchronized block.
 the  synchronization mechanism may change in next revisions, but the goal is stated.


(de)initialization sequence of modules:
the module constructor that runs :
0. fetch add 1 to the initialization atomic and if it was not 0 at the beginning, return.

1.  set the data segment memory accesses (ie. make the read only sections read only)

2. all the dependant modules get initialized .
...
program code: 
 ...
 6.  destroy every static variable in the reverse order of declaration and deinitilize each module that we initialized.
...
* in a dll initialization the loader has different work to do, so the first step is loading, and after the destruction theres unloading.

 
 * each import/export declaration of a module has an import/export symbol of the depenant module identifier 256bit backend hashes, the identifier is as if its a function named with the module name within a `bool __mccabiv1::modules::<name of module and its module fragment>(bool)` that is similar to the the dll of the module `mayde_initialization_fn_offset` function, however it only (de)initializes the module itself, not all the modules in the dll,
 once all the modules in a dll are uninitialized, then `mayde_initialization_fn_offset` can give permission to unload it.
...
// 1 : `dll_comparable_address` vs `no_dll_comparable_address`
 ...
 //if no-interposition and dllexport and `no_dll_comparable_address` pointer is the offset of the function adress relative to offset 0 

 // if its interpositioned and dllexport and `no_dll_comparable_address` , pointer is the offset of the reserved static atomic storage for the address of the function relative to offset 0
 
//if no-interposition and dllexport and `dll_comparable_address` pointer is the offset of the function pointer relative to offset 0 , this function pointer has the function adress relative to offset 0  
 
// if its interpositioned and dllexport and `dll_comparable_address` ,  pointer is the offset of the reserved static atomic storage pointer for the address of the function relative to offset 0, this storage pointer has the reserved static atomic storage for the address of the function relative to offset 0

// if no-interposition and dllimport  pointer is the offset of the function pointer relative to offset 0 
 
// if its interpositioned and dllimport , pointer is the offset of the reserved static atomic storage pointer for the address of the function relative to offset 0
...
// initialized with the function definition 
...
// this is done before the module initialization, `O(total_symbol_count)` time complexity 
...
7. freeze the "write only once in initialization" section to readonly 
...
1. assign all the other address to this address.
3 . return successfully 



// this is done after the module deinitialization `O(total_symbol_count)` time complexity 
...
// how dllmap/loader stores info about the dlls inside it :
`
struct dll_info {
  ...
  struct offset_t {
    ...
    // if dll is inlined this is the offset of the inner dll relative to  the dllmap's offset zero, if it has a path its the offset  from that files beginning
...
- extern "c++" (` represent_cxx`):
 ...
 the function signature, should have types that are not reorderable, templates that are valid in c++, and structures consistent of only fundamental cxx types, this will be more exactly specified in next revisions,
...
requirements afterwards include having the OS loader step, lib unwind if cxx exception is enabled and itanium symbol mangling.
 ...
 very bare bones, only fundamental types, trivial structures and pointers .
...
in the caller, before the call, the context-type gets a chance to capture the protection information of the function if it wants to, to protect against stack overflow, and a minimalistic debug info for the stack trace.
note that capture of backend-hash makes compilation slower because of non elided hash 
in a debugging environment, this can have conditional trap instructions. 
...
in the callee, before the callee code and stack get initialized, the context-type gets a chance to capture the protection information of the function if it wants to, to protect against stack overflow, and a minimalistic debug info for the stack trace.
note that capture of backend-hash makes compilation slower because of non elided hash 
in a debugging environment, this can have conditional trap instructions. 
...
-`operator debug(instruction-pointer/expression-index)context-type`:
 ...
 an implementation defined debugging thread might change a table ( atomic store ) ( the table's atomic pointer pointer can be captured in the meta operator ) to indicate ( atomic load)  that this debug logic should trap or not.
...
- different types of `contract_assert`:
...
- what different types of contract violation semantics do, when violation happens:
 ...
 * note that using e and m necceceraly makes using N not possible, because the addition of e, m and u indicates the bit count and, if bit  count is not devisible by 8, fractional alignment. 
 ...
 - note : 
  some implementations may have a templated non fundamental type that is used to represent custom mantisa and exponents .
...
also, fundamental type of `std::char_t` that represents an ASCII code point.
* note :
we have a non fundamental type of `std::grapheme_t<normalization>` that represents an extended grapheme cluster.
...
but, if implicit contract violation during the constexpr runtime occurs the program is ill-formed, explicit ones are implementation defined if they will throw an exception or be ill-formed, however the default should be ill-formed.
...
however this would only be used if the type is passed by value ( ie. trivially relocatable and is not refrence ), refrence types or pointers also may have preference for specific registers that can have load and store ptr operands ( for example  load effective address in x86)
 it is implementation defined what preferences are used, and if some non fundamental types also have preferences or not ( for example a sso string object may use compiler intrinsics to say that a specific register category of ymm is preferred, however using intrinsics is unsafe(magic) and probably would need other unsafe blocks to indicate non standard abi)
...
each of these 3 are allowed to overlap with each other, because in  an instruction of call or return, only one of them is active.
 ...
 return-value, out parameters, out part of inout parameters, and the context-type are here for example.
 ...
 * note on runtime pointers:
  ...
  if ` ptr1 <= ptr2 ` then `uptr2 - uptr1` is equivalent to  `ptr2 - ptr1`, otherwise the behaviour is unspecified.
...
for the default use of xxhash128, a collision resistance grantee is achieved for less than  2 ^ 32 different declarations. 
...
let b:= a; 
{
  ...
  assert(a==0);
    ...
    // =:= is just like =: and := combined
    // a+1 is evaluated before =:= so a it refers to is 0
     let a=:= a+1;
  assert(a==1);
     let a=:i8 = 2;
    {
...
4. speed :
 ...
 the variable access is often faster, because in architectures like x86, almost 2 kilobytes ( 32 ZMMs ) can be passed between functions via parameters in registers with ease.
...
8. fast enough application logic:
 ...
 the function argument flow specifiers ( like a reference but without stable address on trivial reallocation) the in (akin to const & ) , out (akin to mutable uninitialized& ) and `inout` (akin to mutable& ) parameters are all trivial reallocation and register passed, 
...
// a lambda can only do relocations in its capture in E colon, `fn(){}` is implicitly `fn[pass](){}`, however in c colon we have other types too, copy in c colon is  like `fn[clone](){}` in c++, however with fn before it, but with `fn(){}` the implicit relocation `pass` one, the capture after fn using `[...]` cannot be specified under E colon, so only relocation constructors of lambda is supported,for a copy, we can do a copy in the main scope then relocation into the lambda, this can be thought of making a new function and giving our box to it for usage, so we lost it.
// the implicit lambda for iteration-primitive is the only place where capture is by reference in E colon, and this is also heavily restricted under the iteration-primitive rules
// also because the scope begins and ends predictably, and the caller is conceptually paused until the end of the iteration, this references capture is almost always a pass under implicit borrow rules. 
...
i pridict that the worst common error is an easy "must initialized an out parameter" or "make a copy of a variable and use that instead of passing itself to multiple function parameters"( the reason being that variables are relocated to the parameter when the parameter is created and will be relocated back when the parameter is "destroyed" in c colon land, but, often the only use after-rellocation-error is these, which can be avoided by declaration of a new variable ), which is, simple and far better than lifetimes or memory bugs.
...
however, because of unsafty of non trivial type erasure, and references capture. lambda ,
...
the type can be used within a context with `stdd::interpreter_context_t<JIT-optimization-level>`, its a violation if the interpreter does not match the one that manages the lifetime.
...
the value also would need to be destructible.
